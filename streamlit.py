...
st.markdown("_👉🏽 Created by Don Maggio_")

#Configuracion de side bar
with st.sidebar:
    st.header("Configuración")
    uploaded_file = st.file_uploader("Elija un archivo")

#Condicional para ver la pagina solo si se carga un archivo
if uploaded_file is None:
    st.info("Cargar un archivo para comenzar", icon="⚠️")
    st.stop()
...
#Pre visualizacion del DF
with st.expander("Previsualizar datos cargados"):
    st.dataframe(
        df,
        column_config={"Year" : st.column_config.NumberColumn(format="%d")}
        )

### Visuzalizacion de metodos ###
